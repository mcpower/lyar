...
def is_disabled(userid, questid):
    ...
    if (any(userid == vote.voter_id for vote in all_votes) or
            not not_mine(userid, questid)):
        return "disabled"
    else:
        return " "
...
def insert_new_question(response):
    ...
    if error_message is None:
        ...
        api.Question.create(statement0, statement1, statement2,
                            lie, user_id, name)
        response.redirect("/")
    else:
        response.write(template.render_page("q_create.html", {
...
def vote(response):
    ...
    if current_user.uid == question.creator_id:
        ...
        if vote.vote == question.lie:
            question_correct_votes = len(api.Vote.find_all(qid=question.qid,
                                                           vote=question.lie))
...
def question_handler(response, question_id):
    ...
    if len(api.Vote.find_all(qid=question_id, voter_id=current_user.uid)) > 0:
        ...
        votes = [len(api.Vote.find_all(question_id, vote=num))
                 for num in range(3)]
        total = sum(votes)
        ...
        context["voted"] = True
        current_vote = api.Vote.find_all(qid=question_id,
                                         voter_id=current_user.uid)[0]
